...
class Parce_Data:
    ...
    def validation_line(
            self, line: str,
            keys: List[str]) -> Tuple[str, Tuple[int, int], Dict[str, Any]]:
        ...
        if j == -1:
            j = len(valid_line)
        data_zone = valid_line[i + 1:j].strip().split()
            ...
            try:
                if flag == 0:
                    if line.startswith("nb_drones:") and self.nb_drones == 0:
                        ...
                        print(f"nb_drones {line_nb} {self.nb_drones}")
                    else:
                    ...
                    if self.start_hub is None:
                        ...
                        self.start_hub = Zone(name,x,y,z_type,z_color,z_max)
                    else:
                    ...
                    print(f"connnnnection {line_nb}")
                else:
                    raise ValueError(f"Unknown line format or invalid prefix: '{line.split()[0]}'")
